...
export function closetItemToGarment(item: ClosetItem, userId: string): Garment {
  ...
  const inferSeason = (name: string, category: string): Season => {
    ...
    if (nameLower.includes("wool") || nameLower.includes("sweater") || nameLower.includes("coat") || nameLower.includes("jacket")) {
      return "winter";
    }
    if (nameLower.includes("linen") || nameLower.includes("cotton") || nameLower.includes("light")) {
      return "summer";
    }
    return "all-season";
  };
