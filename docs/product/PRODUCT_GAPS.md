...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| **Incomplete recommendation loop** | `recommendationLogs.log` exists but is never called from the UI. No "shown", "saved", "skipped", or "worn" logging.                               | No analytics or learning from feedback; learning pipeline (issue #27) cannot be built. |
...
| **Manual weather fallback**        | When geolocation is denied, no manual city input (issue #17).                                                                                     | Users in some regions cannot get weather-based recommendations.                        |
| **Score breakdown UI**             | Score breakdown by category exists in service but not expanded in UI (issue #21).                                                                 | Explainability underused.                                                              |
| **Explicit Skip on outfits**       | No explicit "Skip" or "Next" on outfit cards with logging (issue #15).                                                                            | Incomplete feedback signal.                                                            |
