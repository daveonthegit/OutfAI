...
export class OutfitRecommendationService {
  ...
  private static scoreStyleCoherence(garments: Garment[]): number {
    ...
    const extractStyles = (tags: string[]): string[] => {
      const styleKeywords = ["minimalist", "bold", "classic", "trendy", "avant-garde", "casual"];
      return tags.filter(tag => styleKeywords.some(keyword => tag.includes(keyword)));
    };
   ...
   */
  private static scoreOccasionMatching(garments: Garment[], mood: Mood): number {
    ...
    for (const garment of garments) {
      const garmentOccasions = garment.tags.filter(tag =>
        ["casual", "formal", "work", "smart-casual", "night", "weekend"].includes(tag)
      );
  ...
  private static generateReasons(garments: Garment[], mood: Mood): string[] {
    ...
    const allStyles = garments.flatMap(g => 
      g.tags.filter(t => ["minimalist", "bold", "classic", "trendy", "avant-garde"].includes(t))
    );
    const uniqueStyles = [...new Set(allStyles)];
    
    if (uniqueStyles.length === 1) {
      reasons.push(`Cohesive ${uniqueStyles[0]} aesthetic`);
    } else if (uniqueStyles.includes("classic") && (uniqueStyles.includes("bold") || uniqueStyles.includes("minimalist"))) {
      reasons.push("Classic foundation with modern twist");
    }

    // Occasion reasoning
    const allOccasions = garments.flatMap(g =>
      g.tags.filter(t => ["casual", "formal", "work", "smart-casual", "night", "weekend"].includes(t))
    );
    ...
    const versatilePieces = garments.filter(g => g.tags.includes("versatile-high")).length;
